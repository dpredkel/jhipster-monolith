...
export default (state: EntryState = initialState, action): EntryState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_ENTRIES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_ENTRIES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_ENTRIES):
...
export const getSearchEntities: ICrudSearchAction<IEntry> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_ENTRIES,
  payload: axios.get<IEntry>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
