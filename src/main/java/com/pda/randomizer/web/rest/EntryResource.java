...
import com.pda.randomizer.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.pda.randomizer.domain.Entry}.
 */
@RestController
@RequestMapping("/api")
public class EntryResource {
    ...
    public ResponseEntity<List<EntryDTO>> searchEntries(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Entries for query {}", query);
        Page<EntryDTO> page = entryService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
